# Mr. Game & Watch — 1.1.0

## Changes

### Jab / Tilts

**Jab**
jab 1 can now cancel into grab
jab 1 angle: 80 → 120
rapid jab now leads into dash attack

**Forward Tilt**
sped up whole move 1.2x

**Up Tilt**
damage: 8 → 9
faf: 13 → 11
sped up 1.3x
bkb: 30 → 55
kbg: 125 → 90
angle: 100 → 93

**Down Tilt**
angle: 361 → 80
active frames: 5-15 → 5-10
damage: 9 → 10

### Aerial Attacks

**Forward Air**
late hit angle: 361 → 55
angle: 361 → 55

**Back Air**
multihit lowered to just 1 hit into final hit
now has 2 multihits before final hit, no longer kills
multihit now has skb: 10
multihit bkb: 25 → 35
final hit damage: 3 → 12 → 6
final hit kbg: 170 → 100 → 90 → 120
final hit bkb: 30 → 55
final hit angle: 361 → 62
angle: 68 → 365
multihit damage: 3 → 1
multihits: 2 → 4

**Neutral Air**
removed multihit and made move a single hit kill move
bkb: 25
kbg: 107
damage: 16
given back his parachute

**Up Air**
reworked to function as a proper multihit
has been changed to his old nair

**Down Air**
removed stall and fall
sourspot angle: 60 → 270
sweetspot duration: 1 → 4
removed a bug where the meteor hit of his dair was lasting the entire length of the move
spike hitbox: 11-15
sourspot: 15 → 38
spike hitbox active frames: 4 → 1 → 2
late hit duration sped up 1.3x, endlag slowed 0.7x to compensate
size: 4.32/6.24 → 3.5/5
late hit angle: 60 → 80

### Smash Attacks

**Up Smash**
sped startup 1.3x
removed iasa
startup sped up 1.45x

**Down Smash**
swapped sourspot and sweetspot priority
startup sped up 2x

### Special Moves

**Side B**
lowered odds of getting a 1, increased odds of getting a 9

### Other

**Dash Attack**
angle: 120 → 95

**Down Throw**
angle: 270 → 80
bkb: 80 → 90

### Character Parameters

| Parameter | Change |
|-----------|--------|
| Run Speed | 1.553 → 1.63 → 1.62 → 1.7 |
| Ground to Air Momentum | buffed |
| Dash Speed | 1.5 → 1.55 |
| Weight | 75 → 80 |
